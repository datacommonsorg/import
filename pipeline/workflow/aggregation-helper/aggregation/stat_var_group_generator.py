...
from .common import BASE_PROVENANCE_PREFIX, _escape_sql_literal, get_generated_provenance_sql_expr, get_provenance_name
    ...
    def run_stat_var_group(self, import_names: List[str]) -> Optional[bigquery.job.QueryJob]:
        ...
        CREATE OR REPLACE TEMP TABLE StatVarTriple AS (
          ...
          FROM EXTERNAL_QUERY("{conn_id}",
            "SELECT subject_id, predicate, object_id, provenance FROM Edge WHERE predicate IN UNNEST([{sv_predicates_sql}]) AND object_id NOT LIKE '[%' {provenance_filter}"
          ) E
