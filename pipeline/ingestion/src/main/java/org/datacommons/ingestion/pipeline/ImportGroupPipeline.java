...
public class ImportGroupPipeline {
    ...
    public static void main(String[] args) {
        IngestionPipelineOptions options = PipelineOptionsFactory.fromArgs(args).withValidation()
                .as(IngestionPipelineOptions.class);
        Pipeline pipeline = Pipeline.create(options);
        LOGGER.info("Running import group pipeline for import group: {}", options.getImportGroupVersion());

        CacheReader cacheReader = new CacheReader(options.getSkipPredicatePrefixes());
        SpannerClient spannerClient = SpannerClient.builder()
        ...
        String cachePath = CacheReader.getImportGroupCachePath(options.getStorageBucketId(),
                options.getImportGroupVersion());

        buildImportGroupPipeline(pipeline, cachePath, cacheReader, spannerClient);
