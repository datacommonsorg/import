...
Note that if you are importing a dataset where non-numerical StatVar Observations expected (for example, statType is measurementResult and therefore the SVObs values are references), set `--allow-non-numeric-obs-values=true` in the command line invocation.
