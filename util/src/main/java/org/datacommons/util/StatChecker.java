...
public class StatChecker {
  ...
  // key is generated by getSamplePlacesKeys, value is set of place dcids
  ...
  private boolean shouldExtractSeriesInfo(McfGraph.PropertyValues node) {
    ...
    if (shouldGenerateSamplePlaces) {
      ...
      // If this is not a new place type (determined by placekey) and/or the
      // place type has hit the maximum number of sample places, don't add place
      // to sample places map.
      if (samplePlaces.get(placekey).size() >= getSamplePlaceBucketSize(placeDcid)
  ...
  // returns true if the placeDcid is not already in sample places map AND the
  // bucket for this places key is not full.
  /*private boolean shouldAddPlaceToSamplePlacesMap(String placeDcid) {
    String placekey = getSamplePlacesKey(placeDcid);

    boolean bucketNotFull = samplePlaces.get(placekey).size() < getSamplePlaceBucketSize(placeDcid);
    boolean samplePlaceNotAlreadyIncluded = !samplePlaces.get(placekey).contains(placeDcid);

    return bucketNotFull && samplePlaceNotAlreadyIncluded;
  }
  /* */
  ...
  // given a place dcid, checks if it is to be bucketed by length and returns
  // the appropriate samplePlaces key.
